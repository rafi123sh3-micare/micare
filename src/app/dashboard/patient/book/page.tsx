...
// ✅ FIX: Reliable local-timezone YYYY-MM-DD string (avoids UTC shift bug)
...
export default function PatientBook() {
  ...
  async function loadDoctorSchedules() {
    ...
    if (scheduleData && scheduleData.length > 0) {
      ...
      // ✅ FIX: Use toLocaleDateString('en-CA') for reliable YYYY-MM-DD in local timezone
      // This avoids the UTC shift that caused "today" slots to appear on "tomorrow"
      const todayStr = toLocalDateStr(now);
      const isToday = selectedDate === todayStr;

      console.log('todayStr:', todayStr, 'selectedDate:', selectedDate, 'isToday:', isToday);
      ...
      const filteredSlots = scheduleData.filter((s: any) => {
        ...
        // ✅ FIX: Only apply time filter if the selected date is actually today
  ...
  // ✅ FIX: Build dateStr using toLocaleDateString('en-CA') — consistent with todayStr in loadDoctorSchedules
  const getNextDays = () => {
    const days = [];
    const now = new Date();
    ...
    for (let i = 0; i < 14; i++) {
      ...
      // ✅ Use same method as todayStr so selectedDate === todayStr always matches correctly
  ...
  const formatDate = (dateStr: string) => {
    // ✅ FIX: Parse YYYY-MM-DD as local date by splitting manually (avoids UTC midnight shift)
    const [year, month, day] = dateStr.split('-').map(Number);
    const date = new Date(year, month - 1, day); // local time, no UTC offset issue
